...
用法：
    ...
    python pool.py --date 2026-04-30 --days 30 --universe-source local
...
# 趋势类卖出最少持有天数。硬止损、跌破 MA60 不受此限制。
MIN_HOLD_DAYS = 3

# close: 触发日收盘卖出
# next_open: 触发日收盘确认，下一交易日开盘卖出
...
def build_dashboard_df(
    ...
    rows.append(["动态卖出", "开启" if DYNAMIC_EXIT_ENABLED else "关闭", "股票走弱则卖出，否则持有到期"])
    rows.append(["动态卖出最少持有天数", MIN_HOLD_DAYS, "硬止损和跌破MA60不受此限制"])
    rows.append(["动态卖出执行时点", EXIT_SELL_TIMING, "close=收盘卖；next_open=次日开盘卖"])
    rows.append(["硬止损", STOP_LOSS_PCT, "相对买入价亏损达到该比例卖出"])
    rows.append(["移动止盈启动", TRAILING_START_PCT, "盈利达到该比例后启动移动止盈"])
    rows.append(["移动止盈回撤", TRAILING_DRAWDOWN_PCT, "从最高收盘价回撤该比例卖出"])
    ...
    if overall_summary_df is not None and not overall_summary_df.empty:
        ...
        for _, r in tmp.iterrows():
            h = int(r["horizon"])
            rows.append([f"D{h}平均收益", r["avg_ret"], "固定持有口径"])
            rows.append([f"D{h}中位数收益", r["median_ret"], "固定持有口径"])
            rows.append([f"D{h}胜率", r["win_rate"], "固定持有口径"])
            rows.append([f"D{h}沪深300收益", r["bench_ret"], "同期沪深300累计收益"])
            rows.append([f"D{h}平均超额收益", r["avg_excess"], "固定持有平均收益 - 沪深300"])
            rows.append([f"D{h}跑赢沪深300比例", r["outperform_rate"], "固定持有跑赢比例"])
    ...
    if dynamic_exit_summary_df is not None and not dynamic_exit_summary_df.empty:
        ...
        if not all_row.empty:
            r = all_row.iloc[0]
            rows.append(["动态卖出平均最终收益", r.get("avg_final_ret", np.nan), "动态卖出后的最终收益均值"])
            rows.append(["动态卖出中位数最终收益", r.get("median_final_ret", np.nan), "动态卖出后的最终收益中位数"])
            rows.append(["动态卖出胜率", r.get("win_rate", np.nan), "动态最终收益大于0的比例"])
            rows.append(["动态卖出平均持有天数", r.get("avg_hold_days", np.nan), "动态卖出平均持有交易日数"])
            rows.append(["动态卖出平均最高浮盈", r.get("avg_max_ret", np.nan), "动态持有期平均最高浮盈"])
            rows.append(["动态卖出平均最大回撤", r.get("avg_max_dd", np.nan), "动态持有过程平均最大回撤"])
...
def style_worksheet(
    ...
    for col_idx in range(1, max_col + 1):
        ...
        ws.column_dimensions[col_letter].width = min(max(max_len + 2, 10), 26)
