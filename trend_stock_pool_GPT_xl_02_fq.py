...
# 说明：
# 1. 本版本尽量避开东方财富接口。
# 2. 股票列表优先使用 ak.stock_info_a_code_name()。
# 3. 历史行情使用新浪接口 ak.stock_zh_a_daily()。
# 4. 本代码仅用于研究和辅助筛选，不构成投资建议。
# """
...
#     if code.startswith(("83", "87", "88", "43")):
#         return "bj" + code
...
#     注意：
#     这里不使用 stock_zh_a_spot_em()，因为它是东方财富接口。
#     """
#     print("正在获取A股股票列表...")
...
#     # 如果上面失败，尝试新浪A股实时行情接口
#     # 某些 AkShare 版本中可能存在 stock_zh_a_spot()
...
#     # 如果没有 amount，做一个近似。
#     # 注意：这只是兜底方案，真实成交额最好使用接口原始 amount。
...
#     不使用东方财富 stock_zh_a_hist()。
#     改用新浪 stock_zh_a_daily()。
#     """
#     try:
...
#     if pd.isna(row["ret10"]) and row["ret10"] > 0.35:
#         return True
